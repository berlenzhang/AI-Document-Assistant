...
**Switching `llm_provider` (`flan_t5`/`claude`) or any other `.env` value requires a backend restart** — `pydantic-settings` reads `.env` once at process startup (`config.py:settings = Settings()`), and `uvicorn --reload` only watches `.py` files, not `.env`. Editing `.env` alone has no effect until the process is restarted.
...
- `eval/faithfulness_eval.py` reconstructs the actual context block via `LLMService._build_context_block(...)` directly (reading the budget off each provider's own `LLMService` instance), rather than using `Citation.excerpt` (only the first 200 chars — shorter than the real per-chunk budget); judging against the excerpt would understate what the model actually saw and produce false "unsupported" verdicts. `eval/judge.py` itself only does the grading call — it's handed the already-reconstructed context.
